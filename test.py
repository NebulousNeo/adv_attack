...
parser.add_argument('-path','--path',default="../CARLA",type=str)
...
ss= []
print("---IMG READ---")
for img_idx in range(len(filelist)):
    s = re.findall("\d+", filelist[img_idx])[0]
    ss.append(int(s))
ss = np.array(ss)
indexes = ss.argsort()
for img_idx in sorted(ss):
    index = indexes[img_idx]
    f = filelist[index]
    orig_name = f
    gen_img = preprocess_image(orig_name)
    raw_img = preprocess_image(orig_name,(640,1280))
    imgs.append(gen_img)
...
for img_idx in range(len(imgs)):
    angle3.append(model1.predict(imgs[img_idx])[0])

print("---CALCULATE THE DIRECTION COMPLETE---")
print("---IMG WRITE---")
for img_idx in range(len(raw_imgs)):
    #if(i<100):
    #   continue 
    #output.write(str(i)+" " + str(float(angle3[i]))+ "\n")
    #gen_img_deprocessed = draw_arrow2(deprocess_image(imgs[i]),angle3[i])
    gen_img_deprocessed = draw_arrow2(deprocess_image(raw_imgs[img_idx], (640, 1280, 3)), angle3[img_idx], 10)
    imageio.imwrite('./test_output/' + str("%03d" % img_idx) + '.png', gen_img_deprocessed)
